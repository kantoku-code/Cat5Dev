...
Run **`Cat5Dev: Initialize Project (cat5dev.toml)`** from the Command Palette to generate a configuration file template in your workspace root.
...
コマンドパレットから **`Cat5Dev: Initialize Project (cat5dev.toml)`** を実行すると、ワークスペースルートに設定ファイルの雛形が生成されます。
各ルールは個別に有効/無効を切り替えられます：
